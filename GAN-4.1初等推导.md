...
论文架构简单，实验配置很常规；14年那会图像生成好像很缺benchmark，GAN论文连定量分析都没做，也没什么好说的。作者为了提高工作量和专业性，只能抄起数学武器证明这个方法的合理性，这一定程度上弥补了实验的不足。

## 初等方法推导4.1 $p_g = p_{data}$

没学过 **Kullback-Leibler divergence** 和 **Jensen-Shannon divergence** 怎么办？🤔
...
根据论文前面的推导，对于固定的生成器 $G$，最优的判别器 $D^{\ast} (x)$
是： $$D^{\ast} (x) = \frac{p_{data} (x)}{p_{data} (x) + p_g (x)}$$
带入$V (D, G)$，我们得到(4)：
...
把$p_g (x)$和$p_{data} (x)$视为常数，令$y
= p_g (x), z = p_{data} (x)$，实际上$p_g
(x)$是优化变量。
...
$$z \log \left( \frac{2 z}{y + z} \right) \geq z \cdot  \left( 1
   - \frac{y + z}{2 z} \right) = \frac{z - y}{2}$$
